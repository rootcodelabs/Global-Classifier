...
def download_multiple_chunks(dataset_id: str, chunk_ids: List[int]) -> Dict[str, Any]:
    ...
    """
    try:
        log(f"Starting multi-chunk download - Dataset ID: {dataset_id}, Chunks: {chunk_ids}")
        ...
        for chunk_id in chunk_ids:
            ...
            if result["success"]:
                ...
                log(f"✅ Chunk {chunk_id}: {len(chunk_items)} items added to aggregation")
            else:
                failed_chunks.append(chunk_id)
                log(f"❌ Chunk {chunk_id}: Download failed - {result.get('error', 'Unknown error')}")
        ...
        if successful_chunks and download_results:
            ...
            if first_successful:
                ...
                chunk_info = {
                    "original_dataset": original_chunk_info.get("original_dataset", dataset_id),
                    ...
                    "aggregation_range": f"chunks {min(successful_chunks)}-{max(successful_chunks)}" if successful_chunks else "none"
                }
        ...
        log(f"Multi-chunk aggregation completed - Success: {aggregated_payload['success']}")
...
def parse_chunk_ids(chunk_ids_str: str) -> List[int]:
    ...
    try:
        ...
        chunk_ids = [int(chunk_id.strip()) for chunk_id in chunk_ids_str.split() if chunk_id.strip()]
        log(f"Parsed chunk IDs: {chunk_ids}")
        return chunk_ids
    except ValueError as e:
        log(f"Error parsing chunk IDs '{chunk_ids_str}': {str(e)}")
        raise ValueError(f"Invalid chunk IDs format. Expected space-separated integers, got: '{chunk_ids_str}'")

def main():
    """Main function to handle multi-chunk download and aggregation process."""
    parser = argparse.ArgumentParser(description='Download and aggregate multiple chunks from S3')
    parser.add_argument('--dataset-id', required=True, help='Dataset ID')
    parser.add_argument('--chunk-ids', required=True, help='Space-separated chunk IDs (e.g., "1 2 3")')
    parser.add_argument('--output-json', help='Output file path for results JSON')
    
    args = parser.parse_args()
    
    try:
        log(f"Processing multi-chunk request - Dataset: {args.dataset_id}, Chunk IDs: {args.chunk_ids}")
