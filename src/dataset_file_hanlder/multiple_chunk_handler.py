...
class MultiChunkService:
    ...
    def download_single_chunk_from_s3(self, dataset_id: str, chunk_id: int) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"S3Ferry response status for chunk {chunk_id}: {response.status_code}")
            ...
            if response.status_code in [200, 201]:
                ...
                if os.path.exists(local_file_path):
                    logger.info(f"Successfully downloaded chunk {chunk_id} to: {local_file_path}")
    ...
    def download_multiple_chunks(self, dataset_id: str, chunk_ids: List[int]) -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.info(f"Starting multi-chunk download - Dataset ID: {dataset_id}, Chunks: {chunk_ids}")
            ...
            for chunk_id in chunk_ids:
                ...
                if result["success"]:
                    ...
                    logger.info(f"✅ Chunk {chunk_id}: {len(chunk_items)} items added to aggregation")
                else:
                    failed_chunks.append(chunk_id)
                    logger.error(f"❌ Chunk {chunk_id}: Download failed - {result.get('error', 'Unknown error')}")
            ...
            logger.info(f"Multi-chunk aggregation completed - Success: {aggregated_payload['success']}")
