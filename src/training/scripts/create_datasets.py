...
class ScalableDatasetProcessor:
    ...
    def load_data_chunked(self, chunk_size: int = None) -> Iterator[Tuple[List[str], List[str]]]:
    ...
    def create_label_mapping_optimized(self, labels: List[str]) -> Tuple[Dict[str, int], Dict[int, str]]:
    ...
    def split_dataset_large(self, texts: List[str], labels: List[str], 
        ...
        if len(texts) > 100000:
            return self._split_by_indices(texts, labels, test_size, val_size, random_state)
        else:
            return self._split_direct(texts, labels, test_size, val_size, random_state)
    
    def _split_by_indices(self, texts: List[str], labels: List[str], 
                         test_size: float, val_size: float, random_state: int) -> Dict[str, List]:
        ...
        train_indices, val_indices = train_test_split(
            temp_indices, test_size=val_ratio, random_state=random_state,
    ...
    def _split_direct(self, texts: List[str], labels: List[str], 
                     test_size: float, val_size: float, random_state: int) -> Dict[str, List]:
    ...
    def save_splits_chunked(self, splits: Dict, label_to_id: Dict[str, int], 
                           id_to_label: Dict[int, str], metadata: Dict):
    ...
    def _save_large_split(self, file_path: Path, split_data: Dict, label_to_id: Dict[str, int]):
    ...
    def _save_small_split(self, file_path: Path, split_data: Dict, label_to_id: Dict[str, int]):
    ...
    def _save_metadata(self, label_to_id: Dict, id_to_label: Dict, metadata: Dict, splits: Dict):
        """Save metadata files."""
        # Label mappings
    ...
    def process_dataset(self, test_size: float = 0.2, val_size: float = 0.1, 
                       random_state: int = 42):
        """Main processing pipeline optimized for large datasets."""
        logger.info(f"Starting scalable dataset processing for dataset ID: {self.dataset_id}")
        ...
        splits = self.split_dataset_large(texts, labels, test_size, val_size, random_state)
...
def main():
    parser = argparse.ArgumentParser(description='Process large aggregated datasets for training')
