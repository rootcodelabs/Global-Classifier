...
def notify_progress_uploading_to_s3(session_id: int) -> None:
    ...
    try:
        logger.info(f"Calling progress update endpoint: {PROGRESS_UPDATE_URL} with payload={payload}")
...
def process_callback_background(file_path: str, encoded_results: str, session_id: int) -> None:
...
def parse_args():
    ...
    parser.add_argument("--session-id", required=True, help="Session ID for the callback")
...
def main():
    ...
    try:
        ...
        process_callback_background(args.file_path, args.encoded_results, args.session_id)
