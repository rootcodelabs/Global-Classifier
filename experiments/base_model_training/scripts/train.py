...
def load_data(data_path, split=None):
    """
    Load the data from the given path with consistent label mapping.
    """
    if split:
    ...
    if df[label_col].dtype == "object":
        # Define consistent mapping file path
        mapping_file = os.path.join(os.path.dirname(data_path), "label_mapping.json")

        # Try to load existing mapping first
        if os.path.exists(mapping_file):
            print(f"Loading existing label mapping from {mapping_file}")
            with open(mapping_file, "r") as f:
                label_map = json.load(f)
        else:
            # Create consistent label mapping by sorting the unique labels
            unique_labels = sorted(df[label_col].unique())
            label_map = {label: i for i, label in enumerate(unique_labels)}

            # Save the mapping for consistency across splits
            os.makedirs(os.path.dirname(mapping_file), exist_ok=True)
            with open(mapping_file, "w") as f:
                json.dump(label_map, f, indent=2)
            print(f"Created new label mapping: {label_map}")

        # Convert string labels to integers using consistent mapping
        labels = df[label_col].map(label_map).values

        # Check for any unmapped labels
        if np.any(pd.isna(labels)):
            unmapped = df[label_col][pd.isna(labels)].unique()
    ...
    print(
        f"Loaded {len(df)} samples with label distribution: {dict(zip(*np.unique(labels, return_counts=True)))}"
    )
...
def evaluate(model, dataloader, device, num_labels):
    ...
    # write text input,  all_preds, all_labels to a file
    with open("predictions.txt", "w") as f:
        for text, pred, label in zip(dataloader.dataset.texts, all_preds, all_labels):
            f.write(f"{text}\t{pred}\t{label}\n")
...
def train_and_evaluate(args):
    ...
    # show sample of train_dataset
    print(f"Sample from train dataset: {train_texts[0]} -> {train_labels[0]}")

    val_dataset = TextClassificationDataset(
        val_texts, val_labels, tokenizer, max_length=args.max_seq_length
    )
    # show sample of val_dataset
    print(f"Sample from validation dataset: {val_texts[0]} -> {val_labels[0]}")
    test_dataset = TextClassificationDataset(
        test_texts, test_labels, tokenizer, max_length=args.max_seq_length
    )
    # show sample of test_dataset
    print(f"Sample from test dataset: {test_texts[0]} -> {test_labels[0]}")
    ...
    with mlflow.start_run(
        ...
        for epoch in range(args.num_epochs):
            ...
            print(f"Epoch {epoch + 1}/{args.num_epochs} - Validation Metrics:")
        ...
        print("Test Metrics:")
        print(f"Best epoch: {best_epoch + 1}, Best validation F1: {best_val_f1:.4f}")
...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, default=52, help="Random seed for reproducibility"
    )
    parser.add_argument(
