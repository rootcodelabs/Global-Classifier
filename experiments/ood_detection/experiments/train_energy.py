"""
Training script for Energy-based OOD detection model.
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train and evaluate Energy-based OOD detection model"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="outputs/energy",
        help="Directory to save outputs",
    )
    parser.add_argument(
        "--model-dir", type=str, default="models/energy", help="Directory to save model"
    )
    ...
    parser.add_argument(
        "--energy-temp", type=float, default=1.0, help="Temperature for energy scoring"
    )
    parser.add_argument(
        "--energy-margin", type=float, default=10.0, help="Margin for energy-based loss"
    )
    parser.add_argument(
        "--energy-weight", type=float, default=0.1, help="Weight for energy loss term"
    )
    parser.add_argument(
        "--use-energy-loss",
        action="store_true",
        help="Whether to use energy-based loss during training",
    )
...
def prepare_configs(args):
    ...
    training_config.model_config = energy_config

    return training_config, energy_config
...
def train_and_evaluate(
    ...
    """
    Train and evaluate the Energy-based OOD detection model.

    Args:
        config: Training configuration
        model_config: Model configuration
        use_energy_loss: Whether to use energy-based loss during training
        energy_margin: Margin for energy-based loss
    ...
    run_name = f"energy_{model_config.energy_temp}_{random_id:.4f}"
    if use_energy_loss:
        run_name += f"_margin{energy_margin}_weight{energy_weight}"
    ...
    params = {
        ...
        "energy_temp": model_config.energy_temp,
    ...
    if use_energy_loss:
        params.update({"energy_margin": energy_margin, "energy_weight": energy_weight})
    ...
    # Create model
    logger.info("Creating Energy-based model...")
    ...
    if use_energy_loss:
        ...
        loss = EnergyLoss(
            ood_label=-1,  # OOD label for synthetic examples
    ...
    # If using energy loss, we need to prepare special training data with OOD examples
    if use_energy_loss and ood_test_dataset:
        # Create a dataset that includes both ID and OOD examples
        # First, extract a small portion of the OOD data for training
        ood_train_examples = []
        ood_train_labels = []

        # Take a small sample of OOD data (10% of the size of the training data)
        num_ood_train = min(int(num_train_examples * 0.1), 500)
        ood_count = 0

        for batch in ood_test_dataset:
            inputs, _ = batch
            for i in range(len(inputs["input_ids"])):
                if ood_count >= num_ood_train:
                    break
                ood_train_examples.append(
                    {
                        "input_ids": inputs["input_ids"][i].numpy(),
                        "attention_mask": inputs["attention_mask"][i].numpy(),
                    }
                )
                ood_train_labels.append(-1)  # OOD label
                ood_count += 1
            if ood_count >= num_ood_train:
                break

        # Now extract the ID examples
        id_train_examples = []
        id_train_labels = []
        ...
        # Combine ID and OOD examples
        all_examples = id_train_examples + ood_train_examples
        all_labels = id_train_labels + ood_train_labels
        ...
        for i in range(0, len(shuffled_examples), model_config.batch_size):
            batch_examples = shuffled_examples[i : i + model_config.batch_size]
            batch_labels = shuffled_labels[i : i + model_config.batch_size]

            if (
                len(batch_examples) == model_config.batch_size
            ):  # Skip incomplete batches
                # Stack examples
                input_ids = np.stack([ex["input_ids"] for ex in batch_examples])
                attention_mask = np.stack(
                    [ex["attention_mask"] for ex in batch_examples]
                )
        ...
        mixed_dataset = tf.data.Dataset.from_tensor_slices(
            (batched_inputs, batched_labels)
        )
        mixed_dataset = mixed_dataset.unbatch().batch(model_config.batch_size)
        ...
        # Standard training on ID examples only
        history = model.fit(
            train_dataset,
            validation_data=dev_dataset,
            epochs=model_config.epochs,
...
def main():
    ...
    logger.info("Completed Energy-based OOD detection training and evaluation")
