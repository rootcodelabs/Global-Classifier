"""
Training script for combined SNGP + Energy model.
"""
...
import json
import time
from config.config import TRAINING_CONFIG, SNGPEnergyConfig
from data.data_loader import DataLoader
from models.sngp_energy_model import SNGPEnergyModel, SNGPEnergyLoss
from models.sngp_model import ResetCovarianceCallback
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train and evaluate SNGP + Energy model"
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="outputs/sngp_energy",
        help="Directory to save outputs",
    )
    parser.add_argument(
        "--model-dir",
        type=str,
        default="models/sngp_energy",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.9,
        help="Spectral normalization bound",
    )
    parser.add_argument(
        "--gp-hidden-dim",
        type=int,
        default=1024,
        help="Gaussian process hidden dimension",
    )
    parser.add_argument(
        "--gp-scale-random-features",
        action="store_true",
        help="Whether to scale random features in GP layer",
    )
    parser.add_argument(
        "--gp-cov-momentum",
        type=float,
        default=-1.0,
        help="Covariance momentum (-1.0 for exact calculation)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Weight for combining SNGP and Energy scores (0-1)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="ood_detection_sngp_energy",
...
def prepare_configs(args):
    ...
    # Create SNGP+Energy config
    sngp_energy_config = SNGPEnergyConfig(
        name="sngp_energy_model",
        model_type="sngp_energy",
        pretrained_model=args.pretrained_model,
        spec_norm_bound=args.spec_norm_bound,
        gp_hidden_dim=args.gp_hidden_dim,
        gp_scale_random_features=args.gp_scale_random_features,
    ...
    return training_config, sngp_energy_config
...
def train_and_evaluate(
    ...
    """
    Train and evaluate the SNGP+Energy model.
    ...
    run_name = f"sngp_energy_{model_config.spec_norm_bound}_{model_config.energy_temp}_{model_config.alpha}_{random_id:.4f}"
    ...
    mlflow_logger.log_params(
        {
            ...
            "spec_norm_bound": model_config.spec_norm_bound,
            "gp_hidden_dim": model_config.gp_hidden_dim,
            "gp_scale_random_features": model_config.gp_scale_random_features,
            "gp_cov_momentum": model_config.gp_cov_momentum,
    ...
    # Create model
    logger.info("Creating SNGP+Energy model...")
    model = SNGPEnergyModel(
        pretrained_model=model_config.pretrained_model,
        num_labels=num_labels,
    ...
    # Custom loss function that combines cross-entropy with energy-based loss
    loss = SNGPEnergyLoss(
        ood_label=-1,  # OOD label in dataset
        energy_margin=energy_margin,
        energy_weight=energy_weight,
    )
    # Compile model
...
def main():
    ...
    logger.info("Completed SNGP+Energy training and evaluation")
