"""
Training script for SNGP model.
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train and evaluate SNGP model")
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="outputs/sngp",
        help="Directory to save outputs",
    )
    parser.add_argument(
        "--model-dir", type=str, default="models/sngp", help="Directory to save model"
    )
...
def prepare_configs(args):
    ...
    training_config.model_config = sngp_config

    return training_config, sngp_config


def train_and_evaluate(config, model_config, use_gpu=False, log_performance=False):
    """
    Train and evaluate the SNGP model.
    ...
    run_name = (
        f"sngp_{model_config.spec_norm_bound}_{model_config.gp_hidden_dim}_{random_id}"
    )
    ...
    # Create model
    logger.info("Creating SNGP model...")

    model = SNGPModel(
        pretrained_model=model_config.pretrained_model,
        num_labels=num_labels,
        hidden_dims=[768, 512],  # Add a hidden layer
        dropout_rate=0.1,
        spec_norm_bound=model_config.spec_norm_bound,
        gp_hidden_dim=model_config.gp_hidden_dim,
        gp_scale_random_features=model_config.gp_scale_random_features,
    ...
    for batch in test_dataset:
        ...
        if len(preds_np.shape) == 0:  # scalar
            id_predictions.append(preds_np.item())
            id_uncertainties.append(uncertainties_np.item())
        else:  # array
            id_predictions.extend(preds_np.flatten())
            id_uncertainties.extend(uncertainties_np.flatten())
        if len(labels_np.shape) == 0:  # scalar
            id_labels_list.append(labels_np.item())
        else:  # array
            id_labels_list.extend(labels_np.flatten())
    ...
    if ood_test_dataset:
        ...
        for batch in ood_test_dataset:
            ...
            uncertainties_np = uncertainties.numpy()
            if len(preds_np.shape) == 0:  # scalar
                ood_predictions.append(preds_np.item())
                ood_uncertainties.append(uncertainties_np.item())
            else:  # array
                ood_predictions.extend(preds_np.flatten())
                ood_uncertainties.extend(uncertainties_np.flatten())
...
def main():
    ...
    logger.info("Completed SNGP training and evaluation")
